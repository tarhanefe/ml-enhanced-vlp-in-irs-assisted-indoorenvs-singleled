function [neighbours,rmse_list] = knnrun(orientation,height,measurement_count,grid_resolution,train_noise,test_noise,test_count)
    rmse_list = [];
    ...
    for q = 1:test_count
        ...
        dum_power = [];
        for irs = irs_list
            pwr = calcPower(loc,nb,irs,led,test_noise,0);
            powers = pwr.power + sqrt(test_noise)*randn(1);
            dum_power = vertcat(dum_power,powers);
        end
        power = horzcat(power,dum_power);
    end
    ...
    for nbrs = neighbours
        ...
        predictedY = fitrKNN(trainX, trainY, testX, nbrs);
