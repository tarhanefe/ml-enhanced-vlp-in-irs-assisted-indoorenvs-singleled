function rmse_list = nnRegression(model_arch,orientation,height,measurement_count,grid_resolution,train_noise,test_count)
    rmse_list = [];
    ...
    net = feedforwardnet(model_arch);
    %net.trainParam.showWindow = false;
    %net.trainParam.showCommandLine = false;
    %net.layers{1}.transferFcn = 'poslin';  % or 'relu'
    %net.layers{2}.transferFcn = 'poslin';  % or 'relu'
    %net.layers{end}.transferFcn = 'poslin';
    trainX = train_data(:, 1:end-2); 
    trainY = train_data(:, end-1:end);    
    ...
    for noise_var = noise_variances
        ...
        mse = sum(mean((testY_pred' - testY).^2));
        rmse = sqrt(mse);
